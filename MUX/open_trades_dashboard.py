...
def webhook():
    ...
    order_book = client.futures_order_book(symbol=symbol_var, limit=10)

    liquidity = 0
    unit_price = round(unit_price, 2)

    if positionSide_var == "SHORT":

        for bid in order_book["bids"]:
            if float(bid[0]) >= unit_price:  # only consider bids above or equal to the price
                # add the quantity to the total size
                liquidity += float(bid[1])

    if positionSide_var == "LONG":

        for ask in order_book["asks"]:
            if float(ask[0]) <= unit_price:  # only consider asks below or equal to the price
                # add the quantity to the total size
                liquidity += float(ask[1])

    send_list[10] = liquidity
    ...
    if Is_in_list == "Yes":
        ...
        mail_col=['Transaction Hash', 'Wallet Address', 'Position Side', 'Side', 'Token', 'Token Quantity',	'Token Price (USD)', 'Volume traded', 'Timestamp', 'Trader In List', 'Leverage']													
        mail_list=send_list[:10]

        # Set the subject and body of the email
        subject = 'Position increase by a profitable trader'
        ...
        body = f"""
        Position increase by a profitable trader
        {table_format}
        """
